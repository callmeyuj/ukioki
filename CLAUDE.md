...
| **订阅计划** | `/plan/` | 价格计算器（名义订阅）：固定包数 + 口味分配 + 折扣阶梯 → 参考预算 | 🔄 需求定义中 |
...
**当前版本**：需求定义中（文档骨架已建，详见 `plan/CLAUDE.md`）
**定位**：名义订阅，实质**价格计算器**（不含配送周期/订阅管理/支付）
**核心流程**：
- 参考每日包数（calories/ 跳转或手动输入）
- 消费者确定固定总包数
- 自由分配各口味包数
- 按总包数套用折扣阶梯 → 输出参考预算
...
| 优先级 | 功能 | 状态 | 说明 |
|--------|------|------|------|
| **P0** | 订阅计划（plan/） | 🔄 需求定义中 | 价格计算器：固定包数 + 口味分配 + 折扣阶梯 → 参考预算（详见 plan/CLAUDE.md） |
...
- 进度：需求定义接近完成（2026/09/16 更新：折扣阶梯 + 引导策略已确认，详见 plan/CLAUDE.md）
- 下一步：**方案设计**——待定项仅剩：配送费、输出形态细节
- 状态：**🔄 需求定义中 → 待方案设计**
