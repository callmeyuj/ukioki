...
| **订阅计划** | `/plan/` | 价格计算器（名义订阅）：固定包数 + 口味分配 + 折扣阶梯 → 参考预算 | ✅ 需求定义完成 → 待方案设计 |
...
**当前版本**：✅ 需求定义完成（2026/09/16，决策 #1~#25 落定，详见 `plan/CLAUDE.md`）→ 待方案设计
...
| 优先级 | 功能 | 状态 | 说明 |
|--------|------|------|------|
| **P0** | 订阅计划（plan/） | ✅ 需求定义完成 → 待方案设计 | 价格计算器：固定包数 + 口味分配 + 折扣阶梯 → 参考预算（详见 plan/CLAUDE.md） |
...
### 1. 订阅计划模块开发（P0 - 下次优先）
- 范围：新建 plan/ 模块（价格计算器定位）
- 进度：✅ **需求定义完成**（2026/09/16，决策 #1~#25 全部落定：定位、数据来源、折扣阶梯、锚点引导、包数口径、热量校验、猫延展性、运费规则、输出形态）
- 下一步：**方案设计**——页面流程 + 交互细节 + 计算逻辑 + Plan 模式评审（含 calories/ 跳转入口改动评估）
- 状态：**✅ 需求定义完成 → 待方案设计**
