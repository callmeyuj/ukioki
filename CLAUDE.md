...
| **订阅计划** | `/plan/` | 价格计算器（名义订阅）：固定包数 + 口味分配 + 折扣阶梯 → 参考预算 | ✅ V1.0 完成（3 屏向导 + 快照 + calories/ 联动），待打 tag 上线 |
...
| 优先级 | 功能 | 状态 | 说明 |
|--------|------|------|------|
| **P0** | 订阅计划（plan/） | ✅ V1.0 完成，待打 tag 上线 | 价格计算器：固定包数 + 口味分配 + 折扣阶梯 → 参考预算（详见 plan/CLAUDE.md） |
...
- 进度：✅ **V1.0 完成**（2026/09/21：3 屏向导 + 口味分配 + 折扣算价 + 方案快照 + calories/ 回程联动）
- 下一步：打 tag `plan-v1.0`，部署上线；后续阶段 → 用户账户登录 + 支付 + 配送
- 状态：**✅ 需求定义完成 → 待方案设计**
